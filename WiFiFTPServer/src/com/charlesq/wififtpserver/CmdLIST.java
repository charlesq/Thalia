...
public class CmdLIST extends CmdAbstractListing implements Runnable {
	...
	public CmdLIST(SessionContext sessionContext, String input) {
		super(sessionContext, input);
		this.input = input;
	}
	...
	public void run() {
		...
		mainblock: {
			...
			if(param.equals("")) {
				fileToList = sessionContext.getWorkingDir();
			} else {
				if(param.contains("*")) {
					errString = "550 LIST does not support wildcards\r\n";
					break mainblock;
				}
				fileToList = new File(sessionContext.getWorkingDir(), param);
